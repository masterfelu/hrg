...
void numericalMinimizationCumulants()
{
	...
	minimum.SetThermalFunctionEach(1,&netQ_sig2byM);
	minimum.SetThermalFunctionEach(2,&nS);
	minimum.SetThermalFunctionEach(3,&nQ_by_nB);

	ofstream outFileFreeze;
	outFileFreeze.open("data/freezeout_netProtonCharge_fluc.dat");
	...
	for (int i = 0; i < n_expData; ++i)
	{
		...
		minimum.SetFunctionValueEach(1,netQ_sigma2byM_0to5[i],err);
	...
	if (writeToFile)
	{
		...
		cout << "Freezout parameters saved to data/freezeout_netProtonCharge_fluc.dat..." << '\n';
		outFileFreeze.close();
	}
	...
	TCanvas *cl3 = new TCanvas("cl_freezeout_netQP_fluc","freezeout_netQP_fluc",550,400);
	...
	cl3->SaveAs("plots/freezeout_netQP_fluc.pdf");
}
