...
void findChemFreezeoutParam_netLambda_fluc()
{
	int n_expData;

	double *energy_exp, *netKaon_energy_exp, *netLambda_energy_exp;
	...
	inFileNetKaon.open("data/cumulantRatios_netKaon_fit.dat");
	inFileNetLambda.open("data/cumulantRatios_netLambda_fit.dat");
	if (inFileNetKaon && inFileNetLambda)
	{
		inFileNetKaon >> l1;
		inFileNetLambda >> l2;
		...
		energy_exp = new double[l]; netKaon_energy_exp = new double[l]; netLambda_energy_exp = new double[l]; 
		...
		for (int i = 0; i < l; ++i)
		{
			...
			inFileNetLambda >> netLambda_energy_exp[i] >> netLambda_sigma2byM_exp_0to5[i] >> netLambda_sigma2byM_exp_0to5_staterr[i] >> netLambda_sigma2byM_exp_0to5_syserr[i]
			 ...
			 if (netKaon_energy_exp[i] != netLambda_energy_exp[i])
			 {
			 	cout << "Energy value mismatch at line " << i+2 << '\n';
			 }
			 energy_exp[i] = (netKaon_energy_exp[i]+netLambda_energy_exp[i])/2.0;
		}
	}
	else{
	...
	for (int i = 0; i < n_expData; ++i)
	{
		...
		minimum.SetFunctionValueEach(0,netLambda_sigma2byM_exp_0to5[i],err);
	...
	if (writeToFile)
	{
		...
		cout << "Freeze out parameters saved to data/freezeout_netLambda_fluc.dat..." << '\n';
		outFileFreeze.close();
	}
	...
	TCanvas *cl2 = new TCanvas("cl_netLambda_fluc","netLambda_fluc",400,600);
	...
	TCanvas *cl3 = new TCanvas("cl_freezeout_netLambda_fluc","freezeout_netLambda_fluc",550,400);
	...
	cl3->SaveAs("plots/freezeout_netLambda_fluc.pdf");
}

int main()
{
	TApplication app {"app", nullptr, nullptr};
	findChemFreezeoutParam_netLambda_fluc();
